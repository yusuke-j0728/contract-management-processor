...
function runAllTests() {
  console.log('=== CONTRACT MANAGEMENT EMAIL PROCESSOR - COMPREHENSIVE TEST SUITE ===');
  ...
  try {
    // Test 5: Spreadsheet operations
    console.log('\n5️⃣ Testing Spreadsheet Operations...');
    if (CONFIG.ENABLE_SPREADSHEET_LOGGING) {
      testSpreadsheetOperations();
      testResults.spreadsheetOperations = true;
      testResults.passedTests++;
      console.log('✅ Spreadsheet operations test PASSED');
    } else {
      console.log('⚠️ Spreadsheet logging disabled - skipping test');
      testResults.spreadsheetOperations = true;
      testResults.passedTests++;
    }
    
  } catch (error) {
    console.error('❌ Spreadsheet operations test FAILED:', error.message);
  }
  
  try {
    // Test 6: Trigger operations
    console.log('\n6️⃣ Testing Trigger Operations...');
...
function printTestSummary(results) {
  ...
  console.log(`  Spreadsheet Ops:     ${results.spreadsheetOperations ? '✅ PASS' : '❌ FAIL'}`);
    ...
    const message = {
      ...
      attachments: [{
        ...
        fields: [
          ...
          {
            title: 'Spreadsheet記録',
            value: results.spreadsheetOperations ? '✅' : '❌',
            short: true
          },
          {
            title: 'Trigger操作',
            value: results.triggerOperations ? '✅' : '❌',
            short: true
          }
        ],
        footer: 'Contract Management Email Processor - Test Suite',
